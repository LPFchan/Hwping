...
자세한 내용은 `../rhwp-studio/`의 브라우저 앱 소스와 `test/` 페이지를 참조하세요.
...
빌드 스크립트는 먼저 `crates/rhwp`를 `wasm-pack build --target web --release --out-dir ../../pkg`로 빌드한 뒤, `rhwp-studio`를 Firefox 확장용으로 번들링합니다.
...
이 확장은 `rhwp-studio/`를 Firefox WebExtensions API에 맞게 패키징한 것입니다.
