...
- append-only `research/`, `records/decisions/`, and commit-backed execution history
...
Install `SPEC.md`, `STATUS.md`, `PLANS.md`, and `INBOX.md`, but defer stable IDs, decisions, execution history, and commit provenance.

### Option 3: Full Adoption And Migration

Install the full operating model, seed the first decision and execution-history records, and normalize the upstream-intake package to the same provenance conventions.
...
- `INBOX.md`, `research/`, `records/decisions/`, and commit-backed execution history now exist as canonical routing and provenance layers.
- `upstream-intake/` remains canonical, but its reports and templates should follow `UPS-*` naming and provenance metadata.
- New commits should carry `project:`, `agent:`, `role:`, and `commit:` trailers.
