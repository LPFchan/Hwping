...
- Outcome: ship `apps/hwping-electron/` as phase 1 for the desktop shell
- Why this is accepted: a thin Electron shell can reuse the shared renderer and file bridge without forcing the native macOS frontend to be the first desktop milestone
- Expected value: open/save/recent/print/quit/menu actions in a packaged desktop app
- Preconditions: Chrome packaging, shared renderer outputs, and a narrow preload bridge
- Earliest likely start: active now
- Related ids: `DEC-20260420-002`

### Native macOS Shell Phase 2

- Outcome: implement macOS file extension, Quick Look preview, Quick Look thumbnail, Finder, and menu bar integration after Electron stabilizes
- Why this is accepted: macOS integrations are still part of the minimum viable product, but they now follow the Chrome and Electron rollout
- Expected value: native entry points and better adoption on macOS without requiring a full native app shell first
- Preconditions: Electron shell behavior, shared-core outputs, and narrow Apple-platform boundaries
- Earliest likely start: phase 1 proves out
- Related ids: `DEC-20260420-002`, `DEC-20260409-004`
...
- Electron opening lands a document in Hwping without requiring the native macOS shell
...
- `M2 Electron Desktop Shell`: open, save, recent documents, print, and quit/menu IPC around the shared renderer
- `M3 macOS Companion Pack`: file extension, Quick Look preview, Quick Look thumbnails, Finder integration, and menu bar surface
...
- Initiative: keep the Chrome browser lane and Electron desktop shell aligned with the root truth docs
  - Why now: the browser surface and the phase 1 desktop shell are the first ship targets and the repo needs to say that plainly before more work lands
  - Dependencies: `DEC-20260420-002`, `REPO.md`, and the updated product truth docs
  - Related ids: `DEC-20260420-002`
...
- Initiative: keep the Electron preload bridge thin and focused on open/save/recent/print/menu actions
  - Why now: the phase 1 desktop shell should stay small enough that the native macOS shell can still be phase 2
  - Dependencies: shared renderer outputs and stable file-system bridge behavior
  - Related ids: `DEC-20260420-002`
- Initiative: translate the accepted v1 reader quality bar into concrete milestones
  ...
  - Dependencies: browser packaging, Electron shell design, macOS companion design, and shared-core preview quality
  - Related ids: `DEC-20260420-002`, `DEC-20260409-004`

### Mid Term

- Initiative: implement the macOS companion pack
  - Why later: the Chrome MVP and Electron shell should stabilize before the companion surfaces widen
  - Dependencies: `crates/hwping-core/`, `crates/hwping-ffi/`, browser-extension outputs, Electron shell outputs, and companion-surface design work
  - Related ids: `DEC-20260420-002`, `DEC-20260409-004`
...
- Initiative: evaluate whether a direct page renderer should replace or supplement the browser-led/PDF-backed viewing path after v1
  - Why deferred: the current product direction still favors the Chrome-led reader surface, the Electron shell, and stable companion outputs first
